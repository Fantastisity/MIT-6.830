...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        switch (joinOp) {
            case EQUALS: {
                if (t1pkey || t2pkey) card = t1pkey ? card2 : card1;
                else card = Math.max(card2, card1);
                break;
            }
            case NOT_EQUALS:
                card = card1 * card2;
    ...
    public <T> void enumerateSubset(List<T> v, int pos, int k, List<Set<T>> res, Set<T> tmp) {
        if (k == 0) {
            res.add(new HashSet<>(tmp));
            return;
        }
        for (int i = pos, j = v.size(); i <= j - k; ++i) {
            tmp.add(v.get(i));
            enumerateSubset(v, i + 1, k - 1, res, tmp);
            tmp.remove(v.get(i));
        }
    }
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        CostCard opt;
        List<Set<LogicalJoinNode>> s;
        for (int i = 1; i <= n; ++i) {
            s = new ArrayList<>();
            enumerateSubset(joins, 0, i, s, new HashSet<>());
            for (Set<LogicalJoinNode> j : s) {
                opt = null;
                for (LogicalJoinNode r : j) {
                    try { 
                        CostCard c = computeCostAndCardOfSubplan(stats, filterSelectivities, r, j, opt == null ? 
                        Double.MAX_VALUE : opt.cost, pc);
                        if (c == null) continue;
                        if (opt == null || opt.cost > c.cost) opt = c;
                    } catch (Exception e) {}
                }
                if (opt != null) pc.addPlan(j, opt.cost, opt.card, opt.plan);
            }
        }
        
        List<LogicalJoinNode> opt_plan = pc.getOrder(new HashSet<>(joins));
        if (explain) printJoins(opt_plan, pc, stats, filterSelectivities);
    ...
    private CostCard computeCostAndCardOfSubplan(
            ...
            LogicalJoinNode j, Set<LogicalJoinNode> joinSet,
        ...
        if (news.isEmpty()) { // base case -- both are base relations
            ...
            t1card = stats.get(j.t1Alias).estimateTableCardinality(
                    filterSelectivities.get(j.t1Alias));
            leftPkey = isPkey(j.t1Alias, j.f1PureName);

            t2cost = j.t2Alias == null ? 0 : stats.get(j.t2Alias)
                    .estimateScanCost();
            t2card = j.t2Alias == null ? 0 : stats.get(j.t2Alias)
